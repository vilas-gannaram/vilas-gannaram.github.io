...
<a href={href || '#'} on:click={handleClick}>
	<div class="bg-card text-card-foreground flex rounded-lg">
		<div class="flex-none">
			<Avatar.Root class="bg-muted-background dark:bg-foreground m-auto size-12 border">
		...
		<div class="group ml-4 flex-grow flex-col items-center">
			<div class="flex flex-col">
				<div class="flex items-center justify-between gap-x-2 text-base">
					...
					<div class="text-muted-foreground text-right text-xs tabular-nums sm:text-sm">
						{start} - {end || 'Present'}
					</div>
				</div>
				{#if title}
